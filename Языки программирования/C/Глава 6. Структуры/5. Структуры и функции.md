...
[[Языки программирования/C/Глава 6. Структуры|Глава 6. Структуры]] / Структуры и функции

[[Языки программирования/C/Глава 6. Структуры/4. Массивы структур|Назад]] | [[Языки программирования/C/Глава 6. Структуры|Содержание]]
...
## 2. Структура как параметр функции

При передаче структуры в качестве параметра функция получает копию значений структуры:
...
## 3. Указатели на структуру как параметры

При использовании структур в качестве параметров в функции следует учитывать, что при вызове функции для структуры, так же как и для параметров типа `int` или `char`, выделяется память, в которую помещаются значения элементов структуры. То есть структура в функцию передается по значению, а это значит, что переданную в функцию структуру мы изменить не можем.
...
## 4. Структура как результат функции
...
1. Структура передается в функцию по значению — функция получает копию и не может изменить исходную структуру.
2. Для изменения структуры или экономии памяти передается указатель: `void change_person(struct person *user)`, поля — через `->`.
3. Функция может возвращать структуру: `struct person create_person(char *name, int age)`.
4. Псевдоним `typedef struct { ... } person;` упрощает сигнатуры функций.
