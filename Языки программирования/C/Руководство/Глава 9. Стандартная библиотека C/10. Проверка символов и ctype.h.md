...
[[Языки программирования/C/Руководство/Глава 9. Стандартная библиотека C|Глава 9. Стандартная библиотека C]] / Проверка символов и ctype.h

[[Языки программирования/C/Руководство/Глава 9. Стандартная библиотека C/9. Платформо-независимые целочисленные типы|Назад]] | [[Языки программирования/C/Руководство/Глава 9. Стандартная библиотека C|Содержание]] | [[Языки программирования/C/Библиотеки/<ctype.h>|Вперёд]]
...
> [!NOTE]
> Наличие слеша перед символом не обязательно означает управляющую последовательность и может применяться лишь для экранирования, например, `"\\"` или `"'"`.
...
> [!NOTE]
> Обратите внимание, что пробел является печатаемым символом, хотя он и не имеет графического воплощения.
...
1. `isspace()` — пробельные символы (пробел, табуляция, перевод строки, возврат каретки).
2. `isalnum()`, `isalpha()`, `isdigit()`, `isxdigit()` — проверка на цифру/букву и их сочетания.
3. `iscntrl()` — управляющие символы, `isprint()` — печатаемые символы (антипод), `isgraph()` — символы с графическим представлением (пробел не входит), `ispunct()` — знаки пунктуации.
4. `islower()`/`isupper()` — регистр символа, `tolower()`/`toupper()` — перевод в нижний/верхний регистр.
5. Все функции возвращают 0 или ненулевое значение (для каждой функции оно своё).

Полный справочник по функциям и макросам `<ctype.h>`, включая расширения POSIX и GNU (функции с локалью `_l`, `isascii()`, `toascii()`, `_tolower()`, `_toupper()`), — в статье [[Языки программирования/C/Библиотеки/<ctype.h>|<ctype.h>]] главы «Библиотеки».
