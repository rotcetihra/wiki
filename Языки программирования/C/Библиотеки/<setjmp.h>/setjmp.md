...
- **Поведение при передаче NULL:** поведение не определено. Буфер `env` должен быть валидным объектом типа `jmp_buf`.
- **Установление errno:** макрос не устанавливает `errno`.
- **Возвращаемое значение при ошибке:** не применимо — при первом вызове возвращает `0`, при восстановлении через `longjmp` — значение второго аргумента `longjmp` (или `1`, если передано `0`).
- **Многопоточность:** буфер `jmp_buf` не является потокобезопасным. Каждый поток должен использовать собственный буфер. Некорректно передавать `jmp_buf` между потоками.
- **Связанные функции:** `longjmp` — восстановление контекста; `jmp_buf` — тип буфера. В POSIX: `sigsetjmp`/`siglongjmp` — аналоги с сохранением маски сигналов.
