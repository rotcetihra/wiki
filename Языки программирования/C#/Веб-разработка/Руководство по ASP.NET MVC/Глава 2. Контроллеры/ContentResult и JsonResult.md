...
[[Языки программирования/C#|C#]] / [[Языки программирования/C#/Веб-разработка|Веб-разработка]] / [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET MVC|Руководство по ASP.NET MVC]] / [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET MVC/Глава 2. Контроллеры|Глава 2. Контроллеры]] / ContentResult и JsonResult

[[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET MVC/Глава 2. Контроллеры/Результаты действий|Назад]] | [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET MVC/Глава 2. Контроллеры|Содержание]] | [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET MVC/Глава 2. Контроллеры/Переадресация|Вперёд]]
...
Для отправки ContentResult не надо использовать конструктор, так как в контроллере уже определен специальный метод Content(),
...
Одним из наиболее популярных в наше время форматов хранения и передачи данных является формат JSON (JavaScript Object Notation).
...
название свойства объекта, а значением соответственно значение этого свойства. Например: `{"name":"Tom"}`. Здесь "name" является ключом, а "Tom" - значением.


Для отправки объекта в формате json в контроллере имеется метод Json(object obj), который в качестве параметра принимает отправляемый
объект. Например:
...
![Отправка JSON в контроллере ASP.NET Core MVC и C#](https://metanit.com./pics/2.15.png)


Дополнительная версия метода Json() в качестве второго параметра принимает объект, который задает настройки сериализации в формат json.
В качестве такого объекта выступает объект типа JsonSerializerOptions:
...
говорит, что регистр названий свойств не учитывается. А свойство `WriteIndented = true` задает установку отступов перед свойствами для более форматированного вывода:
![Отправка и настройка сериализации JSON в контроллере ASP.NET Core MVC и C#](https://metanit.com./pics/2.16.png)
...
**Источник:** [https://metanit.com/sharp/aspnetmvc/2.6.php](https://metanit.com/sharp/aspnetmvc/2.6.php)
