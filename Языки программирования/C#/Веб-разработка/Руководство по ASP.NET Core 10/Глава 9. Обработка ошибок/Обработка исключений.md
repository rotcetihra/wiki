...
[[Языки программирования/C#|C#]] / [[Языки программирования/C#/Веб-разработка|Веб-разработка]] / [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET Core 10|Руководство по ASP.NET Core 10]] / [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET Core 10/Глава 9. Обработка ошибок|Глава 9. Обработка ошибок]] / Обработка исключений

[[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET Core 10/Глава 8. Состояние приложения. Куки. Сессии/Сессии|Назад]] | [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET Core 10/Глава 9. Обработка ошибок|Содержание]] | [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET Core 10/Глава 9. Обработка ошибок/Обработка ошибок HTTP|Вперёд]]
...
DeveloperExceptionPageMiddleware. Однако вручную нам его не надо добавлять, поскольку оно добавляется автоматически. Так, если мы посмотрим на код класса [WebApplicationBuilder](https://github.com/dotnet/aspnetcore/blob/main/src/DefaultBuilder/src/WebApplicationBuilder.cs),
который применяется для создания приложения, то мы там можем увидеть там следующие строки:
...
В middleware в методе `app.Run()` симулируется генерация исключения путем деления на ноль.
И если мы запустим проект, то в браузере мы увидим информацию об исключении:
![Обработка исключений и UseDeveloperExceptionPage в ASP.NET Core и C#](https://metanit.com./pics/9.1.png)
...
Соответственно middleware из метода `app.UseDeveloperExceptionPage()` не будет перехватывать исключение. А приложение будет возвращать
...
в чем угодно.
![HTTP ERROR 500 в ASP.NET Core и C#](https://metanit.com./pics/9.2.png)
...
Для этих целей можно использовать еще один встроенный middleware - ExceptionHandlerMiddleware , который подключается с помощью метода UseExceptionHandler(). Он перенаправляет
при возникновении исключения на некоторый адрес и позволяет обработать исключение. Например, изменим код приложения следующим образом:
...
из метода `app.Map()`, в котором пользователю будет отправляться некотоое сообщение со статусным кодом 500.
![Error Handling и метод UseExceptionHandler в ASP.NET Core и C#](https://metanit.com./pics/9.3.png)


Однако данный способ имеет небольшой недостаток - мы можем напрямую обратиться по адресу "/error" и получить тот же ответ от сервера. Но
в этом случае мы можем использовать другую версию метода UseExceptionHandler(), которая принимает делегат, параметр которого представляет объект IApplicationBuilder:
...
**Источник:** [https://metanit.com/sharp/aspnet6/9.1.php](https://metanit.com/sharp/aspnet6/9.1.php)
