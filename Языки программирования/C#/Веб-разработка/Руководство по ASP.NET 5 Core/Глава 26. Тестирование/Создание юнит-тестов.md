...
[[Языки программирования/C#|C#]] / [[Языки программирования/C#/Веб-разработка|Веб-разработка]] / [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core|Руководство по ASP.NET 5 Core]] / [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 26. Тестирование|Глава 26. Тестирование]] / Создание юнит-тестов

[[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 26. Тестирование/Создание проекта юнит-тестов. Добавление xUnit|Назад]] | [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 26. Тестирование|Содержание]] | [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 26. Тестирование/Фреймворк Moq и moq-объекты|Вперёд]]
...
Метод `IndexViewNameEqualIndex` проверяет название вызываемого представления с помощью вызова Assert.Equal.
А метод `IndexViewDataMessage` проверяет значение по ключу "Message" в словаре ViewData.
![Проект юнит-тестов в ASP.NET Core](https://metanit.com./pics/unittest7.png)
...
Модель тестов Arrange-Act-Assert представляет целую парадигму тестирования, которая используется многими
фреймворками юнит-тестов:
...
Equal(expected, result): метод сравнивает результат теста в виде значения `result` и ожидаемое значение
`expected` и подтверждает их равенство

-

NotEqual(expected, result): метод сравнивает результат теста в виде значения `result` и ожидаемое значение
`expected` и подтверждает их неравенство
...
![Запуск юнит-тестов в ASP.NET Core](https://metanit.com./pics/unittest4.png)


После этого откроется окно Test Explorer или обозреватель тестов. Если все нормально, то обозреватель тестов сигнализирует
нам зеленым цветом, что все тесты успешно пройдены. Если же нет, то не пройденные тесты будут отмечены красным цветом.


В данном случае после запуска тестов мы увидим, что один тест - IndexViewDataMessage не пройден. Потому что мы ожидаем, что `ViewData["Message"]` будет иметь значение
"Hello world!". Однако в методе Index контроллера HomeController передается совсем другое значение: `ViewData["Message"] = "Hello!"`. То есть
...
![Unit test in ASP.NET Core MVC](https://metanit.com./pics/unittest6.png)
...
**Источник:** [https://metanit.com/sharp/aspnet5/22.3.php](https://metanit.com/sharp/aspnet5/22.3.php)
