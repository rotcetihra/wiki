...
[[Языки программирования/C#|C#]] / [[Языки программирования/C#/Веб-разработка|Веб-разработка]] / [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core|Руководство по ASP.NET 5 Core]] / [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 4. Конфигурация|Глава 4. Конфигурация]] / Основы конфигурации

[[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 3. Сервисы и Dependency Injection/Singleton-объекты и scoped-сервисы|Назад]] | [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 4. Конфигурация|Содержание]] | [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 4. Конфигурация/Нефайловые провайдеры конфигурации|Вперёд]]
...
Данное руководство устарело. Актуальное руководство: [Руководство по ASP.NET Core 7](https://metanit.com/sharp/aspnet6/)Последнее обновление: 10.11.2019
...
Для создания объекта `IConfigurationRoot` применяется метод `Build()` класса ConfigurationBuilder.
...
`{"firstname", "Tom"}` - ключ настройки "firstname", а значение - "Tom". Далее
...
Выше мы явным образом создавали объект IConfiguration, устанавливая для него источник конфигурации и добавляя различные конфигурационные настройки.
...
можем также получить через конструктор класса Startup:
...
![Конфигурация по умолчанию](https://metanit.com./pics/config1.png)
...
Файл appsettings.Development.json, который находится в проекте по умолчанию (провайдер JsonConfigurationProvider).
Формально приложение подхватывает файл с именем appsettings.{environment}.json, где environment - название стадии, на которой находится проект. Но поскольку
по умолчанию для проекта задается стадия "Development" (то есть проект находится в процессе разработки), а в проекте определяется файл appsettings.develoment.json, то собственно
он и подхватывается.
...
Например, в данном случае значения из appsettings.develoment.json перекроют значения с теми же ключами из appsettings.json.
...
**Источник:** [https://metanit.com/sharp/aspnet5/2.6.php](https://metanit.com/sharp/aspnet5/2.6.php)
