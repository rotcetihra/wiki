...
[[Языки программирования/C#|C#]] / [[Языки программирования/C#/Веб-разработка|Веб-разработка]] / [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core|Руководство по ASP.NET 5 Core]] / [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 8. ASP.NET Core MVC. Первое приложение|Глава 8. ASP.NET Core MVC. Первое приложение]] / Первое приложение. Добавление моделей и базы данных

[[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 8. ASP.NET Core MVC. Первое приложение/Добавление MVC в пустой проект|Назад]] | [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 8. ASP.NET Core MVC. Первое приложение|Содержание]] | [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 8. ASP.NET Core MVC. Первое приложение/Создание контроллера и инициализатора базы данных|Вперёд]]
...
Создадим первое простенькое приложение на ASP.NET Core MVC, которое призвано дать понимание, как работает данная платформа. Вначале создадим новый проект.
Назовем его MobileStore. В качестве шаблона проекта выберем Web Application (View-Model-Controller):
![Веб-сайт на ASP.NET Core MVC](https://metanit.com./pics/mvc6.png)


Наше приложение будет очень простым и будет представлять магазин смартфонов. Какие основные функции у любого магазина -
предоставлять товар на продажу и принимать заказы на товар. В связи с этим мы можем выделить прежде всего две модели: модель смартфона и модель заказа.
...
После определения моделей надо выбрать хранилище данных для этих моделей. Мы будем использовать MS SQL Server. Для работы с MS SQL Server компания Microsoft
...
В данном случае для взаимодействия с MS SQL Server через Entity Framework нам нужен пакет
Microsoft.EntityFrameworkCore.SqlServer. Поэтому добавим данный пакет через пакетный менеджер NuGet:
![Microsoft.EntityFrameworkCore.SqlServer](https://metanit.com./pics/mvc23.png)


Чтобы взаимодействовать с базой данных нам нужен контекст данных. Причем Entity Framework Core использует подход Code First, при котором нам надо сначала определить
модели и контекст данных, а потом уже исходя и этих моделей и класса контекста будет создаваться бд и все ее таблицы.
...
Чтобы создать контекст, нам надо унаследовать новый класс от класса DbContext. Свойства наподобие
`public DbSet<Phone> Phones { get; set; }` помогают получать из БД набор данных определенного типа (например, набор объектов Phone). Фактически
каждое свойство DbSet будет соотноситься с отдельной таблицей в базе данных.


По умолчанию у нас база данных отсутствуют. Поэтому в конструктор MobileContext определен вызов `Database.EnsureCreated()`, который
при отсутствии базы данных автоматически создает ее. Если база данных уже есть, то ничего не происходит.


Чтобы подключаться к базе данных, нам надо задать параметры подключения. Для этого изменим файл appsettings.json. По умолчанию он содержит только настройки
логгирования:
...
Об этом говорит параметр `Server=(localdb)\\mssqllocaldb`. Ну а сама база данных будет называться `mobilestoredb`.
...
**Источник:** [https://metanit.com/sharp/aspnet5/3.2.php](https://metanit.com/sharp/aspnet5/3.2.php)
