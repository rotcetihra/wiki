...
[[Языки программирования/C#|C#]] / [[Языки программирования/C#/Веб-разработка|Веб-разработка]] / [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core|Руководство по ASP.NET 5 Core]] / [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 24. Производительность и кэширование|Глава 24. Производительность и кэширование]] / Атрибут ResponseCache

[[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 24. Производительность и кэширование/Кэширование с помощью MemoryCache|Назад]] | [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 24. Производительность и кэширование|Содержание]] | [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 24. Производительность и кэширование/Сжатие ответа|Вперёд]]
...
Данное руководство устарело. Актуальное руководство: [Руководство по ASP.NET Core 7](https://metanit.com/sharp/aspnet6/)Последнее обновление: 20.12.2019
...
Атрибут ResponseCache представляет еще один инструмент кэширования. Он предполагает установку в ответе клиенту
...
не выполняет кэширование на сервере! Его суть - только в установке отправляемых заголовков.


Заголовок кэширования - это обычные заголовки, которые определяются спецификацией протокола HTTP и которые позволяют управлять кэшированием.
То есть это заголовки Cache-Control, Pragma и Vary.
...
не равно true. Duration добавляет к заголовку `Cache-Control` значение `max-age` с устанавливаемым временным промежутком в секундах
...
NoStore: определяет, будет ли ответ кэшироваться. Если равен `true`, то ответ не кэшируется, а значения свойств
`Location` и `Duration` игнорируются. Кроме того, заголовку `Cache-Control` добавляется значение `no-store`
...
Для практического применения возьмем проект по типу ASP.NET Core Web App (Model-View-Controller) и применим атрибут к какому-нибудь методу контроллера данный атрибут.
Сначала отключим любое кэширование:
...
Здесь атрибут применяется к стандартному методу Index. Значение `NoStore =true` добавляет к заголовку Cache-Control значение `no-store`,
а значение `Location =ResponseCacheLocation.None` добавляет к Cache-Control значение `no-cache`


Теперь обратимся в браузере к этому методу:
![Response Caching ASP.NET MVC Core](https://metanit.com./pics/caching3.png)
...
Мы можем перезагрузить страницу, и опять будет идти обращение к серверу, поскольку кэширование не выполняется.
Теперь изменим атрибут:
...
Теперь будет выполняться кэширование, так как я явным образом использовал значение `Location =ResponseCacheLocation.Any` (оно применяется по умолчанию,
...
![](https://metanit.com./pics/caching4.png)
...
![Кэширование в ASP.NET Core](https://metanit.com./pics/caching5.png)
...
профиль кэширования. Для этого перейдем к методу `ConfigureServices` класса Startup и изменим его следующим образом:
...
Каждый профиль кэширования представлен объектом CacheProfile из пространства имен _Microsoft.AspNetCore.Mvc_. Для
добавления профиля применяется метод `options.CacheProfiles.Add()`, в который передается название профиля и собственно профиль.
...
**Источник:** [https://metanit.com/sharp/aspnet5/14.2.php](https://metanit.com/sharp/aspnet5/14.2.php)
