...
При обработке запроса система маршрутизации автоматически выдергивает из строки запроса данные для параметров маршрута, не зависимости от их содержимого,
...
а другой параметр начинался строго с определенного символа. И для этого нам надо задать ограничения маршрута.


Для задания ограничений маршрутов применяется одна из перегруженных версий метода `MapRoute()` класса RouteBuilder. Например, нам надо
...
С помощью последнего параметра устанавливаются ограничения маршрута. И если мы направим приложению запрос Book/Index,
то маршрут не будет обрабатываться связанным с ним обработчиком, так как имя первого параметра "controller" попадает под ограничение. А запрос Home/Index нормально был бы обработан.
![Route Constraints in ASP.NET Core](https://metanit.com./pics/routing13.png)
...
Кроме этого класса ASP.NET Core предоставляет еще ряд классов ограничений, которые находятся в пространстве имен
[Microsoft.AspNet.Routing.Constraints](https://github.com/aspnet/AspNetCore/tree/master/src/Http/Routing/src/Constraints)
...
OptionalRouteConstraint: определяет ограничение для необязательного параметра. И если параметр имется в запросе, то вступает в силу
внутреннее ограничение, задаваемое через свойство InnerConstraint класса OptionalRouteConstraint
...
С помощью специального класса CompositeRouteConstraint можно установить сложное ограничение, которое будет включать несколько
простых:
...
В данном случае устанавливается, что параметр `action` должен одновременно состоять только из алфавитных символов и также должен иметь
...
ASP.NET Core позволяет также указывать ограничения прямо при определении параметров поле двоеточия в виде сокращений, это так называемые
строчные ограничения (inline constraints):
...
Ограничение `regex` фактически применяет класс RegexRouteConstraint. Все подобные встраиваемые ограничения называются по имени классов без суффикса
`RouteConstraint`.
...
**Источник:** [https://metanit.com/sharp/aspnet5/11.3.php](https://metanit.com/sharp/aspnet5/11.3.php)
