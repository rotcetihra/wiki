...
Чтобы создать собственное ограничение, нужно реализовать интерфейс IRouteConstraint с одним единственным методом Match, который имеет
...
будет пропускать запросу по некоторому url. Итак, добавим в приложение следующий класс:
...
Здесь мы говорим, что если запрашиваемый ресурс совпадает со значением свойства `httpContext.Request.Path`, то запрос не будет
...
В случае выше было определено общее ограничение, которое применялось в целом к запросу. Но также можно задавать и ограничения для
отдельных параметров. Допустим, создадим ограничение по должности:
...
В методе `Match()` с помощью параметра `routeKey` мы получаем параметр маршрута, для которого действует ограничение. А словарь
`values` содержит набор значений маршрута. Используя выражение `values[routeKey]` мы можем получить значение для параметра и что-то сделать с ним.
...
Теперь для соответствия маршруту параметр id должен иметь одно из следующих значений: "admin", "director" или "accountant". То есть строка запроса
могла бы выглядеть так: http://localhost:59406/Home/Index/admin
...
**Источник:** [https://metanit.com/sharp/aspnet5/11.7.php](https://metanit.com/sharp/aspnet5/11.7.php)
