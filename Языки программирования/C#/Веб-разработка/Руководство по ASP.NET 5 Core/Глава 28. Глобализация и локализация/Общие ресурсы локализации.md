...
В прошлых темах для каждой сущности использовались свои ресурсы: для представлений - одни, для контроллеров - другие, для атрибутов модели - третьи.
...
Затем определим для него общие файлы ресурсов. Для этого в проекте в папку Resources добавим несколько файлов ресурсов для тех языков, которые мы хотим использовать.
...
то файлы ресурсов будут иметь имя SharedResource.[код_культуры].resx. Путь к данным файлам ресурсов должен отражать путь к классу SharedResource
внутри проекта (как и в случае с ресурсами для контроллеров и представлений).
...
![Общие ресурсы локализации в ASP.NET Core](https://metanit.com./pics/localization26.png)


Например, тестовый файл ресурсов для русскоязычной локали:
![](https://metanit.com./pics/localization27.png)


И тестовый файл для английского языка:
![](https://metanit.com./pics/localization28.png)


Теперь как мы эти ресурсы можем использовать в приложении? В контроллере мы можем передать данные ресурсы через конструктор
посредством параметра типа IStringLocalizer<SharedResource>:
...
Ключевым моментом здесь является создание объекта IStringLocalizer для ресурса SharedResource с помощью фабрики IStringLocalizerFactory
в методе AddDataAnnotationsLocalization:
...
![Локализация с SharedResource в ASP.NET Core MVC](https://metanit.com./pics/localization29.png)
...
**Источник:** [https://metanit.com/sharp/aspnet5/28.8.php](https://metanit.com/sharp/aspnet5/28.8.php)
