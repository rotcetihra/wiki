...
Для локализации строк в представлении применяется сервис IViewLocalizer, который по своему действию аналогичен сервису IStringLocalizer.
Встроенная реализация этого интерфейса - класс ViewLocalizer в зависимости от пути к локализуемому представлению пытается найти нужный файл ресурсов.


При локализации представлений применяются те же соглашения по именованию, что и при локализации контроллеров. То есть имя файла ресурсов, предназначенного для
представления, должно отражать его путь в проекте. Например, пусть нам надо локализовать представление Views/Home/Index.cshtml.
В этом случае ViewLocalizer будет искать для него файл ресурсов по следующим путям:
...
![Локализация представлений в ASP.NET Core](https://metanit.com./pics/localization11.png)


Как видно из скриншота, папка ресурсов в данном случае называется Resources, и в ней с прошлой темы остались ресурсы для локализации контроллеров.
Использование структуры папок позволяет избежать мешанины в одной папке ресурсов для локализуемых сущностей и должным образом их структуризировать.
...
![Ресурсы представлений в ASP.NET Core MVC](https://metanit.com./pics/localization12.png)


А аналогичный ресурс для русского языка выглядит следующим образом:
![Локализация представлений на языки в ASP.NET Core MVC](https://metanit.com./pics/localization13.png)
...
![View localization in ASP.NET Core](https://metanit.com./pics/localization14.png)


Передадим другую культуру, например, англоязычную:
![View localization in ASP.NET Core MVC](https://metanit.com./pics/localization15.png)
...
**Источник:** [https://metanit.com/sharp/aspnet5/28.5.php](https://metanit.com/sharp/aspnet5/28.5.php)
