...
[[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 28. Глобализация и локализация/Общие ресурсы локализации|Назад]] | [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 28. Глобализация и локализация|Содержание]] | [[Языки программирования/C#/Веб-разработка/Руководство по ASP.NET 5 Core/Глава 29. SignalR Core/SignalR Core. Первое приложение|Вперёд]]
...
В предыдущих статьях для хранения ресурсов, используемых при локализации, применялись файлы ресурсов resx. Однако мы также можем использовать для хранения ресурсов
любое хранилище, например, базу данных. Рассмотрим как это сделать.
...
А затем добавим в папку Models класс контекста данных LocalizationContext, через который мы будем работать с базой данных:
...
Теперь нам надо создать инфраструктуру, которая бы извлекала из базы данных через контекст нужный ресурс. Для этого добавим в корень проекта
новый класс EFStringLocalizer:
...
С помощью индексаторов возвращаем из базы данных по ключу объект Resource, из которого формируем экземпляр класса LocalizedString.
Для обращения к базе данных через встроенный механизм внедрения зависимостей получаем в конструкторе контекст данных.
...
Фабрика EFStringLocalizerFactory будет определена как синглтон (так как нет смысла создавать для каждого запроса отдельный объект фабрики),
...
При создании объекта IStringLocalizer инфраструктурой MVC у фабрики будут вызываться методы Create, которые собственно и будут возвращает
объект EFStringLocalizer. И этот объект потом будет использоваться для локализации.


Итоге получится следующий проект:
![Localization in ASP.NET Core and Entity Framework Core](https://metanit.com./pics/localization32.png)
...
С аннотациями данных все сложнее, так как для аннотаций данных необходимо создать через фабрику объект IStringLocalizer. Но чтобы использовать нужную фабрику,
надо установить зависимость для IStringLocalizerFactory:
...
![Локализация из базы данных в ASP.NET Core](https://metanit.com./pics/localization30.png)
![Локализация с помощью Entity Framework Core в ASP.NET Core MVC](https://metanit.com./pics/localization31.png)
...
**Источник:** [https://metanit.com/sharp/aspnet5/28.9.php](https://metanit.com/sharp/aspnet5/28.9.php)
