...
В то же время стоит отметить, что для перебора коллекции через foreach в принципе необязательно реализовать интерфейс IEnumerable. Достаточно 
...
В данном случае реализуем интерфейс  `IEnumerator<string>` , соответственно в свойстве Current нам надо возвратить объект string. В этом случае при 
переборе в цикле foreach перебираемые объекты будут автоматически представлять тип  string :
...
**Источник:** [https://metanit.com/sharp/tutorial/4.11.php](https://metanit.com/sharp/tutorial/4.11.php)
