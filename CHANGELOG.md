...
- Урок 11 «<stdlib.h>»: подробно расписан раздел о `strtod`/`strtof`/`strtold` (формат разбираемого числа, шестнадцатеричная форма и экспонента `p`/`P`, `INF`/`NAN`, `endptr`, `HUGE_VAL` и `errno`), добавлен раздел об исключительных ситуациях и обработке ошибок (переполнение, память, окружение, неопределённое поведение `atoi`/`div`/`abs(INT_MIN)`, `exit`/`abort`, `qsort`/`bsearch`, `rand`/`srand`); добавлена дата обновления.
